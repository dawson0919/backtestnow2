/**
 * scripts/seed-crypto-1d.mjs
 * Fetches all available 1D Binance klines for crypto assets and upserts into Supabase.
 * Run: node scripts/seed-crypto-1d.mjs
 * Note: 1D bars are fewer (~3,000 for BTC/ETH back to 2017), runs fast.
 */
...
async function fetchPage(symbol, endTime) {
  const params = new URLSearchParams({ symbol, interval: '1d', limit: String(PAGE) })
...
async function main() {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/historical_data?select=asset_id,timestamp&timeframe=eq.1D`,
