/**
 * scripts/seed-crypto-4h.mjs
 * Fetches all available 4H Binance klines for crypto assets and upserts into Supabase.
 * Run: node scripts/seed-crypto-4h.mjs
 */
...
async function fetchPage(symbol, endTime) {
  const params = new URLSearchParams({ symbol, interval: '4h', limit: String(PAGE) })
...
async function main() {
  ...
  // 顯示最終統計
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/historical_data?select=asset_id,timestamp&timeframe=eq.4H`,
